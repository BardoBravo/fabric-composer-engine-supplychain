...
describe('Starting tests for supplychainModel', function() {
  ...
  it('website title should be supplychainModel', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('supplychainModel');
    })
  });

  it('network-name should be fabric-composer-engine-supplychain@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('fabric-composer-engine-supplychain@0.0.1.bna');
    });
  });

  it('navbar-brand should be supplychainModel',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('supplychainModel');
    });
  });
